...
class BridgeRunner(L.LightningModule):
    ...
    def on_test_start(self):
        ...
        # Load mask for evaluation - DESATIVADO porque não temos pasta mask
        # if self.eval_mask:
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.global_rank == 0:
            # Para RGB (C, H, W), manter os canais; para grayscale squeeze
            if all_pred.shape[2] == 1:
                # Grayscale: (batch, device, 1, h, w) -> (batch*device, h, w)
                h, w = x0.shape[-2:]
                self.test_samples.extend(list(zip(
                    slice_indices.flatten().tolist(),
                    all_pred.reshape(-1, h, w).cpu().numpy())))
            else:
                # RGB: (batch, device, 3, h, w) -> (batch*device, 3, h, w)
                c, h, w = x0.shape[-3:]
                self.test_samples.extend(list(zip(
                    slice_indices.flatten().tolist(),
                    all_pred.reshape(-1, c, h, w).cpu().numpy())))
